        ...
        <article className="custom-article">
          <div className="mainarticle">
            {this.props.feedData ? this.props.feedData.map(feed => {
              return <div className="text-left mt-3"><h2>{ feed.title }</h2><div><img src="default.jpg" className="img-fluid my-4"/></div><div>{ feed.contents }</div><hr className="feed-hr my-4" /></div>
            }) : '데이터 불러오는 중'}
          </div>
        </article>
      </div>
